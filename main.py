...
path = 'C:\\Users\\uie30005\\OneDrive - Continental AG\\Relatórios diários\\'
...
def main():
    ...
    for i in files:
        df = pd.read_excel(path + i,
                           sheet_name='Base',
                           usecols=range(0, 14), skiprows=range(0, 1), engine='openpyxl')
        print("Copying data: " + i)
        '''with pd.option_context('display.max_rows', None, 'display.max_columns',
                               None):  # more options can be specified also
            print(df.to_string())'''
        lista = list(df['Item'].isnull().values)
        print(lista)
        cont = 0
        Index = []
        for i in lista:
    ...
    for i in dfs:
        print(i)
        filename = r'geral.xlsx'
        append_df_to_excel(filename, i, sheet_name='Sheet1', index=False)
